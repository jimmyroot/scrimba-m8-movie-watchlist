...
const Db = async () => {
    ...
    const addMovieToDB = async movieID => {
        try {
            ...
            console.error(`Something went whilst attempting to import movie data. The error was: ${e}`)
        }
    }
            ...
            await runTransaction(db, async (transaction) => {
                ...
                try {
                    if (movieToRemove) {
                        ...
                        if (modal) modal.show('Something went wrong, was the movie already removed?')
                    }
                }
                catch (e) {
                    console.error(`Couldn't remove the movie ${movieID} from list ${listPath}. The error was: ${e}`)
                }                  
            })
        }
        catch (e) {
    ...
    const createAccount = async user => {
        try {
            ...
            await runTransaction(db, async transaction => {
                ...
                const accountDoc = await transaction.get(doc(db, 'accounts', user.uid))
            ...
            if (modal) modal.show(`Something went wrong while creating the user. The error was: ${e}`)
        }
    }
